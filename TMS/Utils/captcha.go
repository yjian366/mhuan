...
//
//var (
//	dpi      = flag.Float64("dpi", 60, "screen resolution in Dots Per Inch")
...
//	c.SetHinting(font.HintingFull)
//
//	fontWidth := (captcha.width-4)/ len(text)
//
//
//	for i,s := range text{
//
...
//		c.SetSrc(image.NewUniform(UseBlackColor()))
//		c.SetFontSize(fontSize)
//
//		//设置字体
//		f,err := RandFontFamily()
...
//		x := int(fontWidth)*i+2
//		y := captcha.height/2 + int(fontSize/4)
//
//		pt := freetype.Pt(x, y)
//
